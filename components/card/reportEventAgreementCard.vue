  ...
  <div class="report-event-agreement-card">
    ...
    <v-col cols="12" class="d-flex flex-column"
      ...
      <v-col cols="12" class="d-flex justify-end">
        <v-btn outlined color="red" elevation="1" class="px-8 ml-2" @click="changeVisibilityCommentBox">
          لغو
        </v-btn>
        <v-btn outlined color="blue" elevation="1" class="px-8" @click="sendComment(reportData.id)">
    ...
    <v-col class="col-12" v-if="reportData.description_hidden && isAssesor">شرح مخفی: <span>{{
      reportData.description_hidden }}</span></v-col>
    <v-col cols="12" class="d-flex justify-end" v-if="type === 'M' && !isAssesor && !reportData.is_eval">
      <v-dialog v-model="dialog" width="500">
        ...
        <div class="feedback-modal">
          <Feedback :feedbacks-array="feedbackQuestions" :meeting-id="reportData.id" :steps="steps"
    ...
    async sendComment(id) {
      if (this.comment.length > 0) {
        try {
          ...
          const response = await this.$axios.post(routes.opinionOnEvent, {
            event: id,
